...
We can see that the snps ids are exactly the same found in our dataset.
...
First, we create an empty ped file structure that we will fill up with the data.
...
```{r plink_version, echo=is.unix, eval=is.unix}
run_shell('echo "$PWD"')
run_shell("plink --version") #PLINK v1.90b6.21 64-bit (19 Oct 2020)
```
...
```{r make_dir_plink, echo=is.unix, eval=is.unix}
run_shell('echo "$PWD"')
run_shell('mkdir -p "$PWD"/data/Output/plink/0-Data')
```

The next step step will be to convert the files obtained into binary files to speed up the computation.

```{r binary_files, echo=is.unix, eval=is.unix}
run_shell('plink --file "$PWD"/data/Input/raw_data --make-bed --out "$PWD"/data/Output/plink/0-Data/binary_data', ignore.stdout = !verbose)
```
...
```{r missing_plink}
run_shell('echo "$PWD"')
run_shell('mkdir -p "$PWD"/data/Output/plink/1-QC/1-Missingness')
run_shell('plink --bfile "$PWD"/data/Output/plink/0-Data/binary_data --missing --out "$PWD"/data/Output/plink/1-QC/1-Missingness/missing', ignore.stdout = !verbose)
run_shell('plink --bfile "$PWD"/data/Output/plink/0-Data/binary_data --geno 0.2 --make-bed --out "$PWD"/data/Output/plink/1-QC/1-Missingness/missing_1', ignore.stdout = !verbose)
run_shell('plink --bfile "$PWD"/data/Output/plink/1-QC/1-Missingness/missing_1 --mind 0.2 --make-bed --out "$PWD"/data/Output/plink/1-QC/1-Missingness/missing_2', ignore.stdout = !verbose)
run_shell('plink --bfile "$PWD"/data/Output/plink/1-QC/1-Missingness/missing_2 --geno 0.02 --make-bed --out "$PWD"/data/Output/plink/1-QC/1-Missingness/missing_3', ignore.stdout = !verbose)
run_shell('plink --bfile "$PWD"/data/Output/plink/1-QC/1-Missingness/missing_3 --mind 0.02 --make-bed --out "$PWD"/data/Output/plink/1-QC/1-Missingness/missing_4', ignore.stdout = !verbose)
```
...
```{r}
run_shell('echo "$PWD"')
run_shell('mkdir "$PWD"/data/Output/plink/1-QC/2-MAF')
run_shell('plink --bfile "$PWD"/data/Output/plink/1-QC/1-Missingness/missing_4 --maf 0.05 --make-bed --out "$PWD"/data/Output/plink/1-QC/2-MAF/minor_allele', ignore.stdout = !verbose)
```
...
```{r}
run_shell('echo "$PWD"')
run_shell('mkdir "$PWD"/data/Output/plink/1-QC/3-HWE')
run_shell('plink --bfile "$PWD"/data/Output/plink/1-QC/2-MAF/minor_allele --hwe 1e-6 --make-bed --out "$PWD"/data/Output/plink/1-QC/3-HWE/hardy_weinberg', ignore.stdout = !verbose)
run_shell('plink --bfile "$PWD"/data/Output/plink/1-QC/3-HWE/hardy_weinberg --hwe 1e-10 include-nonctrl --make-bed --out "$PWD"/data/Output/plink/1-QC/3-HWE/hardy_weinberg_1', ignore.stdout = !verbose)
```
...
```{r indep_SNPs}
run_shell('echo "$PWD"')
run_shell('mkdir "$PWD"/data/Output/plink/1-QC/4-Het')
run_shell('plink --bfile "$PWD"/data/Output/plink/1-QC/3-HWE/hardy_weinberg_1 --exclude "$PWD"/data/Input/inversion.txt --range --indep-pairwise 50 5 0.2 --out "$PWD"/data/Output/plink/1-QC/4-Het/indepSNP', ignore.stdout = !verbose)
```

Once we've done that we can compute the heterozygosity rates

```{r hete_rates}
run_shell('plink --bfile "$PWD"/data/Output/plink/1-QC/3-HWE/hardy_weinberg_1 --extract "$PWD"/data/Output/plink/1-QC/4-Het/indepSNP.prune.in --het --out "$PWD"/data/Output/plink/1-QC/4-Het/Het_check', ignore.stdout = !verbose)
```
...
```{r remove_het_outliers}
run_shell('plink --bfile "$PWD"/data/Output/plink/1-QC/3-HWE/hardy_weinberg_1 --remove "$PWD"/data/Output/plink/1-QC/4-Het/fail-het-qc.txt --make-bed --out "$PWD"/data/Output/plink/1-QC/4-Het/heterozigosity', ignore.stdout = !verbose)
```
...
The next step in the analysis would be to examine all possible interactions of the 75 studied SNPs with the covariates Age75, Sex & E4status.
