...
# 1. Genotyped data set preparation ------------------------------------------
  ...
  ## Subsetting and extracting input data into useful data structures
    ...
    variables <- c("LGCCODE", "Diag", "SexLett", "Age", "Centre", "LGC_E4.")
...
# 2. Imputed dataset preparation ----------------------------------------------
  ...
  if (imputated) {
    ...
    imput_variables <- c("dummyid", "DiagName", "sex", "Age.to.Use", 
                         "Age75", "E4status")
...
# 5. Diagnose possible problems and perform data conversions -------------
  ...
  ## Unexpleined level in the E4status variable (coming from Rotterdam dataset)
...
# 
  ...
  list_of_snp_pairs <- vector(mode = "list", length = nrow(snp_snp))
  
  for (i in seq_len(nrow(snp_snp))) {
    snp_1 <- snp_snp$model1[i]
    snp_2 <- snp_snp$model2[i]
  ...
  snps1 <- unique(snp_snp$snp1)
  models1 <- unique(snp_snp$model1)
  ...
  snp2_per_model <- lapply(unlist(interactions_list), function(x) {unique(snp_snp$snp2[snp_snp$model1 == x])})
  ...
  for (snp1 in names(interactions_list)) {
    for (model1 in names(interactions_list[[snp1]])) {
      ...
      for (i in seq_along(snps_2)) {
        ...
        snp2_models <- snp_snp$model2[snp_snp$model1 == model1 & snp_snp$snp2 == snp2]
  ...
  # master_list <- store_possible_interactions(.master_list = master_list, pairs = list_of_snp_pairs)
  
  for (snp in names(interactions_list)) {
    for (dataset in names(master_list)) {
      master_list[[dataset]][[snp]]$Interactions$SNP <- interactions_list[[snp]]
    }
  }
  ...
  for (i in seq_along(master_list)) {
    ...
    for (snp_pair in list_of_snp_pairs) {
      ...
      significance <- master_list[[i]][[snp1]][["Interactions"]][["SNP"]][[snp1_model]][[snp2]][[snp2_model]][["Significant"]]
      for (x in seq_along(significance)) {
        if (significance[x] == TRUE) {
          ...
          result <- master_list[[i]][[snp1]][["Interactions"]][["SNP"]][[snp1_model]][[snp2]][[snp2_model]]
          print(result)
          if (names(master_list)[i] == "All") {
            significant_snp_pairs <- append(significant_snp_pairs, result$Name[x])
          }
        }
      }
    }
